...
const calculateMetrics = async (userId) => {
    ...
    const recentTransactions = user.transactions.filter((transaction) => transaction.timestamp >= thirtyDaysAgo);
    const totalSpent30Days = recentTransactions.reduce((total, transaction) => total + transaction.amount, 0);
...
const getInsight = (riskScore, daysToZero) => {
    if (riskScore > 75)
        return "Critical: Extreme spending volatility. Your runway is dangerously short.";
    if (riskScore > 50)
        return "Warning: Behavior indicates rising risk. Defer major purchases.";
    if (daysToZero < 15)
        return "Alert: Less than two weeks of runway left based on current trends.";
    if (riskScore < 20 && daysToZero > 60)
        return "Optimal: Behavior is stable. Runway is healthy.";
